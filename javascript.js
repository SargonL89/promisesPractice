...
function aceptarRegistro() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    promesa = new Promise( async (resolve, reject) => {
      regExName.test(nombre) && regExLastName.test(apellido) && regExEmail.test(email) && regExPassword.test(contrasena) ? 
        ...
        reject(console.log("Error! no es por acá"));
    });
  })
  ...
  .catch(!regExName.test(nombre) ? 
    ...
    : mostrarError("No pudimos completar el registro", "Se produjo un error inesperado")
  );
}
